...
typedef struct CXPLAT_CONNECTION CXPLAT_CONNECTION;
...
#define TLS_EXTENSION_TYPE_CXPLAT_TRANSPORT_PARAMETERS                0xffa5  // Host Byte Order
...
typedef struct CXPLAT_TLS_CONFIG {
    ...
    //
    CXPLAT_TLS_RECEIVE_TICKET_CALLBACK_HANDLER ReceiveResumptionCallback;
...
typedef struct CXPLAT_TLS_PROCESS_STATE {
    ...
    //
    CXPLAT_PACKET_KEY_TYPE ReadKey;

    //
    // The highest key available for writing TLS data with.
    //
    CXPLAT_PACKET_KEY_TYPE WriteKey;
    ...
    //
    CXPLAT_PACKET_KEY* ReadKeys[CXPLAT_PACKET_KEY_COUNT];

    //
    // All the keys available for encrypting packets with.
    //
    CXPLAT_PACKET_KEY* WriteKeys[CXPLAT_PACKET_KEY_COUNT];
...
void
(CXPLAT_API CXPLAT_SEC_CONFIG_CREATE_COMPLETE)(
