...
struct CryptTest : public ::testing::TestWithParam<int32_t>
{
    ...
    struct QuicKey
    {
        ...
        QuicKey(CXPLAT_AEAD_TYPE AeadType, const uint8_t* const RawKey) : Ptr(NULL) {
            CXPLAT_STATUS Status = QuicKeyCreate(AeadType, RawKey, &Ptr);
    ...
    struct QuicHash
    {
        ...
        QuicHash(
            ...
            CXPLAT_STATUS Status = QuicHashCreate(HashType, Salt, SaltLength, &Ptr);
            if (Status == CXPLAT_STATUS_NOT_SUPPORTED) {
...
TEST_F(CryptTest, WellKnownClientInitial)
{
    ...
    VERIFY_CXPLAT_SUCCESS(QuicPacketKeyUpdate(PacketKey, &NewPacketKey));
...
TEST_P(CryptTest, HashWellKnown)
{
    ...
    const uint16_t OutputLength = QuicHashLength((CXPLAT_HASH_TYPE)HASH);
