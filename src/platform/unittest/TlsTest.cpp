...
protected:
    ...
    static const CXPLAT_CREDENTIAL_CONFIG* SelfSignedCertParams;
    ...
    void SetUp() override
    {
        ...
        CXPLAT_CREDENTIAL_CONFIG ClientCredConfig = {
            CXPLAT_CREDENTIAL_TYPE_NONE,
            CXPLAT_CREDENTIAL_FLAG_CLIENT,
            NULL,
            NULL
        };
        VERIFY_CXPLAT_SUCCESS(
    ...
    struct TlsContext
    {
        ...
        void InitializeServer(
            const CXPLAT_SEC_CONFIG* SecConfig,
            ...
            )
        {
            ...
            Config.SecConfig = (CXPLAT_SEC_CONFIG*)SecConfig;
    ...
    private:
        ...
        ProcessData(
            _In_ CXPLAT_PACKET_KEY_TYPE BufferKey,
    ...
    public:
        ...
        CXPLAT_BUFFER ResumptionTicket {0, nullptr};
            ...
            )
        {
            if (PeerState == nullptr) {
                ...
                //
                uint32_t Zero = 0;
                return ProcessData(CXPLAT_PACKET_KEY_INITIAL, nullptr, &Zero, ExpectError, DataType);
            }
            ...
            do {
                ...
                CXPLAT_PACKET_KEY_TYPE PeerWriteKey;
                ...
                if (PeerState->BufferOffset1Rtt != 0 && StartOffset >= PeerState->BufferOffset1Rtt) {
                    ...
                    PeerWriteKey = CXPLAT_PACKET_KEY_INITIAL;
    ...
    struct PacketKey
    {
        CXPLAT_PACKET_KEY* Ptr;
        PacketKey(CXPLAT_PACKET_KEY* Key) : Ptr(Key) {
        ...
        )
    {
        ...
        ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
        ASSERT_NE(nullptr, ServerContext.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);

        Result = ClientContext.ProcessData(&ServerContext.State, FragmentSize);
        ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
        ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_COMPLETE);
        ASSERT_NE(nullptr, ClientContext.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);
...
const CXPLAT_CREDENTIAL_CONFIG* TlsTest::SelfSignedCertParams = nullptr;
...
#ifndef CXPLAT_DISABLE_0RTT_TESTS
...
TEST_F(TlsTest, HandshakesInterleaved)
{
    ...
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
    ASSERT_NE(nullptr, ServerContext1.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);

    Result = ServerContext2.ProcessData(&ClientContext2.State);
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
    ASSERT_NE(nullptr, ServerContext2.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);

    Result = ClientContext1.ProcessData(&ServerContext1.State);
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_COMPLETE);
    ASSERT_NE(nullptr, ClientContext1.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);

    Result = ClientContext2.ProcessData(&ServerContext2.State);
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_DATA);
    ASSERT_TRUE(Result & CXPLAT_TLS_RESULT_COMPLETE);
    ASSERT_NE(nullptr, ClientContext2.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);
...
TEST_P(TlsTest, One1RttKey)
{
    ...
    PacketKey ServerKey(ServerContext.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]);
    PacketKey ClientKey(ClientContext.State.ReadKeys[CXPLAT_PACKET_KEY_1_RTT]);
...
TEST_P(TlsTest, KeyUpdate)
{
    ...
    if (PNE) {
        ...
        // key structs.
        //
        UpdateWriteKey->HeaderKey = ServerContext.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]->HeaderKey;
        ServerContext.State.WriteKeys[CXPLAT_PACKET_KEY_1_RTT]->HeaderKey = NULL;

        UpdateReadKey->HeaderKey = ClientContext.State.ReadKeys[CXPLAT_PACKET_KEY_1_RTT]->HeaderKey;
        ClientContext.State.ReadKeys[CXPLAT_PACKET_KEY_1_RTT]->HeaderKey = NULL;
    }
