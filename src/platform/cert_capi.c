    ...
    )
{
    ...
    CertificateNames = CXPLAT_ALLOC_PAGED(Length);
    ...
    )
{
    ...
    *NewCertificate = (CXPLAT_CERTIFICATE*)CertCtx;
    ...
    )
{
    ...
    if (CredConfig->Type == CXPLAT_CREDENTIAL_TYPE_CERTIFICATE_HASH) {
    ...
    } else if (CredConfig->Type == CXPLAT_CREDENTIAL_TYPE_CERTIFICATE_CONTEXT) {
        if (CredConfig->CertificateContext == NULL) {
            Status = CXPLAT_STATUS_INVALID_PARAMETER;
        } else {
            *NewCertificate = (CXPLAT_CERTIFICATE*)CredConfig->CertificateContext;
            Status = CXPLAT_STATUS_SUCCESS;
        }
...
Exit:
    ...
    return (CXPLAT_CERTIFICATE*)LeafCertCtx;
}
