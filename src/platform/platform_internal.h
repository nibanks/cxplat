...
#define CXPLAT_DISABLED_BY_FUZZER_START
#define CXPLAT_DISABLED_BY_FUZZER_END
...
{
    if (InAddr->si_family == CXPLAT_ADDRESS_FAMILY_INET) {
...
{
    CXPLAT_DBG_ASSERT(InAddr->si_family == CXPLAT_ADDRESS_FAMILY_INET6);
    if (IN6_IS_ADDR_V4MAPPED(&InAddr->Ipv6.sin6_addr)) {
        OutAddr->si_family = CXPLAT_ADDRESS_FAMILY_INET;
