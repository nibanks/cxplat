...
{
    CXPLAT_CREDENTIAL_CONFIG* Params =
        HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(CXPLAT_CREDENTIAL_CONFIG) + sizeof(CXPLAT_CERTIFICATE_HASH));
    if (Params == NULL) {
        return NULL;
    }

    Params->Type = CXPLAT_CREDENTIAL_TYPE_CERTIFICATE_CONTEXT;
    Params->Flags = CXPLAT_CREDENTIAL_FLAG_NONE;
