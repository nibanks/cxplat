...
function CMake-Generate {
    ...
    $Arguments += " -DCXPLAT_TLS=" + $Tls
    $Arguments += " -DCXPLAT_OUTPUT_DIR=" + $ArtifactsDir
    if (!$DisableLogs) {
        $Arguments += " -DCXPLAT_ENABLE_LOGGING=on"
    }
    if ($SanitizeAddress) {
        $Arguments += " -DCXPLAT_ENABLE_SANITIZERS=on"
    }
    if ($DisableTools) {
        $Arguments += " -DCXPLAT_BUILD_TOOLS=off"
    }
    if ($DisableTest) {
        $Arguments += " -DCXPLAT_BUILD_TEST=off"
    }
    if ($DisablePerf) {
        $Arguments += " -DCXPLAT_BUILD_PERF=off"
    }
    if ($IsLinux) {
        $Arguments += " -DCMAKE_BUILD_TYPE=" + $Config
    }
    if ($DynamicCRT) {
        $Arguments += " -DCXPLAT_STATIC_LINK_CRT=off"
    }
    if ($PGO) {
        $Arguments += " -DCXPLAT_PGO=on"
    }
    if ($Platform -eq "uwp") {
        $Arguments += " -DCMAKE_SYSTEM_NAME=WindowsStore -DCMAKE_SYSTEM_VERSION=10 -DCXPLAT_UWP_BUILD=on -DCXPLAT_STATIC_LINK_CRT=Off"
    }
    if ($ToolchainFile -ne "") {
    ...
    if ($RandomAllocFail) {
        $Arguments += " -DCXPLAT_RANDOM_ALLOC_FAIL=on"
    }
    $Arguments += " ../../.."
